//! sqlite3-shell — interactive command-line interface for sqlite3-rs.
//!
...
fn main() {
    ...
    let conn = match sqlite3::Connection::open(db_path) {
        Ok(c) => c,
        Err(e) => {
    ...
    println!("SQLite3-rs v{} — targeting SQLite 3.53.x", env!("CARGO_PKG_VERSION"));
...
fn handle_dot_command(cmd: &str, _conn: &sqlite3::Connection) {
    match cmd {
        ".help" => {
