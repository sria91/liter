...
use liter_ast::*;
use liter_vdbe::{Opcode, P4, Vdbe, VdbeOp};
use std::sync::Arc;
...
impl<'a> Compiler<'a> {
    ...
    fn compile_aggregate_select(
        ...
        if let Some(sorter) = group_by_cursor {
            ...
            for agg in &aggs {
                if let Expr::Function { args: liter_ast::FunctionArgs::List(exprs) | liter_ast::FunctionArgs::Distinct(exprs), .. } = agg {
                    for e in exprs {
        ...
        if let Some(sorter) = group_by_cursor {
            ...
            for (agg, dest_reg) in &agg_regs_assigned {
                if let Expr::Function { name, args, .. } = agg {
                    let argc = match args {
                        liter_ast::FunctionArgs::List(exprs) | liter_ast::FunctionArgs::Distinct(exprs) => exprs.len(),
                        _ => 0,
                    };
                    let arg_start = if argc > 0 {
                        let start = self.vm.alloc_reg();
    ...
    fn compile_expr(
        ...
        match expr {
            ...
            Expr::Unary { op: liter_ast::UnaryOp::Minus, operand } => {
