...
use sqlite3_ast::{ColumnDef, Stmt, ResultColumn};
use sqlite3_resolve::{Resolver, ResolveError};
...
fn test_resolve_star_expansion() {
    ...
    if let Stmt::Select(select) = &stmts[0] {
        if let sqlite3_ast::SelectBody::Simple(simple) = &select.body {
            ...
            let names: Vec<String> = simple.result_columns.iter().filter_map(|c| {
                if let ResultColumn::Expr { expr: sqlite3_ast::Expr::Column { name, table, .. }, .. } = c {
